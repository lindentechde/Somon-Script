...
export class PrometheusExporter {
  ...
  exportMetrics(
    ...
    if (stats) {
      this.addStatsMetrics(lines, stats, timestamp);
    }

    if (circuitBreakers) {
      this.addCircuitBreakerMetrics(lines, circuitBreakers, timestamp);
    }

    if (additionalMetrics) {
      this.addAdditionalMetrics(lines, additionalMetrics, timestamp);
    }
  ...
  /**
   * Add module system statistics metrics
   */
  private addStatsMetrics(lines: string[], stats: ModuleSystemStats, timestamp: number): void {
    // System info
